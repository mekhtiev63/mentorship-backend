...
func mapDomain(err error) *apperror.Error {
	...
	case errors.Is(err, domain.ErrAlreadyCancelled), errors.Is(err, domain.ErrAlreadyDeleted):
		return apperror.New(apperror.KindConflict, "conflict", err.Error())
	case errors.Is(err, domain.ErrInvalidTimeRange), errors.Is(err, domain.ErrTitleRequired):
