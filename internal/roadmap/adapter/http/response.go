...
func mapDomain(err error) *apperror.Error {
	...
	case errors.Is(err, domain.ErrHasProgress):
		return apperror.New(apperror.KindConflict, "has_progress", err.Error())
	case errors.Is(err, domain.ErrCannotPublish):
		return apperror.New(apperror.KindValidation, "cannot_publish", err.Error())
	case errors.Is(err, domain.ErrTitleRequired), errors.Is(err, domain.ErrURLRequired),
		errors.Is(err, domain.ErrInvalidMaterialType), errors.Is(err, domain.ErrInvalidStatus),
		errors.Is(err, domain.ErrInvalidReorder), errors.Is(err, domain.ErrInvalidSortOrder),
