...
// OutboxRepo reads bonus-related outbox events.
...
// ListPendingAchievementGranted returns pending achievement.granted events.
func (r *OutboxRepo) ListPendingAchievementGranted(ctx context.Context, limit int) ([]domain.OutboxMessage, error) {
	const q = `
		SELECT id, event_name, payload, created_at
		FROM outbox_events
		WHERE status = 'pending' AND event_name = $1
		ORDER BY created_at ASC
		LIMIT $2
	`
	rows, err := r.pool.Query(ctx, q, domain.AchievementGranted, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// MarkDone marks outbox processed.
func (r *OutboxRepo) MarkDone(ctx context.Context, id string) error {
	const q = `UPDATE outbox_events SET status = 'done', processed_at = now() WHERE id = $1 AND status = 'pending'`
	_, err := db(ctx, r.pool).Exec(ctx, q, id)
	return err
}

// OutboxRecorder records events.
